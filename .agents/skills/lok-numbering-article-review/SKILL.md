...
- Vollständige Betriebsnummer steht **nur** in `model.type` (z. B. Dampf-Präfix «39 1052-8», «38 2566-8»), `model.number` ist `null`, und Baureihe plus Rest sind **eindeutig** splittbar ohne Widerspruch zu `description` / `source.url` erster Zeile bzw. URL-Slug.
- Oder: **Digital-/Sound-Variante** (z. B. Artikelnummer / Kategorie / Text eindeutig Decoder-Digital) mit `electricSystem` noch **DC-Analog**, während die **erste Beschreibungszeile** nur Digital-Betrieb nennt und die **Analog-Schwester** derselben Modellfamilie (`manufacturer` + dokumentierte Gruppe) klar **DC-Analog** bleibt → `electricSystem` auf **DC-Digital** setzen (kein Analogmodell umdeklarieren; **AC-Varianten** separat prüfen).

**Nicht** Auto-Fix: jede neue Baureihe raten, SNCF/PKP-Mischfälle, **Triebzug- und Set-Artikel** ohne eindeutige Einzel-Lok-Nummer (langer `model.type`, `model.number` null, keine splittbare Betriebsnummer in URL erster Zeile), Texte mit OCR-Müll ohne zweite Quelle. Diese Fälle in **Pass 1** als Finding («Set/Serie, Schwester ggf. prüfen»), nicht raten.
